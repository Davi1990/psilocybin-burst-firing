...
def compute_firing_rate_for_periods(spike_times, spike_clusters, unit_ids,
    ...
    for unit_id in unit_ids:
        ...
        results['unit_id'].append(unit_id)
        results['brain_area'].append(brain_area)
        results['location'].append(location)
        results['total_spikes'].append(total_spikes)
        results['firing_rate'].append(firing_rate)
        results['rest_duration'].append(total_duration)
        results['bin'].append(bin_label)
...
def analyze_session(file_path, output_dir):
    ...
    for bin_idx in range(n_effect_bins):
        ...
        effect_results.append(fr_df)

    all_bins_df = pd.concat(baseline_results + effect_results, ignore_index=True)

    all_bins_df.to_csv(os.path.join(output_dir, f'{filename}_prestim_firing_rate_timecourse.csv'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default='output_prestim_firing_rate')
