...
the effect window — default 40–60 min post-injection).
...
Usage
-----
  python distribution_layers_thalamus_hpf_firing_rate_40_60_IR.py \\
...
def load_all_data(base_dir='.'):
    ...
    for treatment, folder in [('Psilocybin', 'psilocybin'),
        ...
        files = sorted(glob.glob(f'{base_dir}/{folder}/*_firing_rate_timecourse.csv'))
...
# ============================================================================
# DELTA COMPUTATION  (per-unit pct change from baseline, effect window)
# ============================================================================
...
# ============================================================================
# RUN ONE PLOT GROUP (cortical / thalamic / hpf) — firing_rate metric
# ============================================================================
...
def main():
    ...
    parser.add_argument('--out_dir',  default='firing_rate_plots_40_60')
    parser.add_argument('--excel_out', default=None,
                        help='Excel output path (default: <out_dir>/firing_rate_stats_<window>.xlsx)')
    ...
    xlsx_path = args.excel_out or os.path.join(args.out_dir, f'firing_rate_stats_{win_tag}.xlsx')
