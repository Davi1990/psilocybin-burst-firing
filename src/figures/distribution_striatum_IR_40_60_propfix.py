"""
Distribution Plots with Half-Violin Overlay — Striatum, 40-60 min Post-Injection
=================================================================================
PROP_BURSTING STATISTICAL FIX — see RAW-PROP-1-style finding in
Burst analysis/DEBUG_LOG.md (same bug independently present here as in
distribution_layers_thalamus_hpf_40_60.py). Only compute_prop_bursting_deltas()
differs from distribution_striatum_IR_40_60.py: prop_bursting is now computed
as a SESSION-LEVEL aggregate (fraction of units bursting per str_nucleus/
session/bin) with an effectively session-only bootstrap (unit_id set equal
to session_id), exactly matching heatmap_hier_bootstrap.py and the Methods
text. The previous version computed a per-unit fraction-of-time-bursting
score and ran it through the standard two-level (session, unit) hierarchical
bootstrap instead. burst_fraction/burst_rate/mean_burst_size are untouched.
...
-------------------
  Striatum (1 row)
       All striatal units (CP, ACB, LSX, STR, etc.) lumped into a single
       'STR' distribution

Metrics
-------
  burst_fraction, burst_rate, mean_burst_size  → absolute delta (effect − baseline) per unit
  prop_bursting                                → absolute change in bursting fraction

Time window: mean across effect_40-45, effect_45-50, effect_50-55, effect_55-60
(configurable via --window_start/--window_end, default 40-60)

Rationale
---------
Companion to distribution_layers_thalamus_hpf_IR.py / distribution_hippocampus_40_60_IR.py.
Striatum (STR) is treated as a single major region elsewhere (heatmap_hier_bootstrap.py);
this script reproduces that single-region view as its own distribution plot, kept
separate so the global EFFECT_WINDOW change does not affect the cortical-layer /
thalamic-group / hippocampal-nuclei panels.

Usage
-----
  python distribution_striatum_IR.py \
      --data_dir output_prestim_burst_new --nboots 1000
"""
...
# Cortical/thalamic classification helpers below are retained because annotate()
# uses them to derive 'cortical_row' / 'thalamic_group' columns — those panels
# are not plotted here (hippocampus only), but the columns are harmless.
...
# ── thalamic group map (from heatmap_hpf_thalamic_nuclei.py) ────────────────
THALAMIC_GROUP_MAP = {
...
# ── Hippocampal nuclei (retained for annotate(), not plotted by this script) ─
HPF_NUCLEUS_MERGE = {'ProS': 'Subicular complex', 'SUB': 'Subicular complex'}
HPF_ROW_ORDER = ['CA1', 'CA2', 'CA3', 'DG', 'Subicular complex']

# ── Striatum (single lumped row — the only panel plotted by this script) ────
STR_ROW_ORDER = ['STR']
...
def get_str_nucleus(base_area):
    """Lump all striatal subregions into a single 'STR' label."""
    return 'STR'
...
def annotate(data):
    ...
    # STR subregion
    def _str_nuc(r):
        if r['major_region'] != 'STR':
            return None
        return get_str_nucleus(r['base_area'])
    data['str_nucleus'] = data.apply(_str_nuc, axis=1)
    ...
    print("\n  Unit counts by str_nucleus:")
    sc = data.dropna(subset=['str_nucleus']).groupby(['str_nucleus', 'treatment'])['unit_id'].nunique().unstack(fill_value=0)
    if not sc.empty:
        print(sc.to_string())
...
def compute_prop_bursting_deltas(data):
    ...
    heatmap_hpf_thalamic_nuclei.py exactly: for each (str_nucleus, session,
    ...
    (The previous version grouped by ['session_id', 'unit_id'] instead of by
    region+session -- see the RAW-PROP-1-style finding logged in
    Burst analysis/DEBUG_LOG.md for distribution_layers_thalamus_hpf_40_60.py,
    the same bug pattern independently present here.)
    ...
    merged = eff_sess.merge(baseline, on=[gc, 'session_id', 'treatment'], how='left')
    merged['prop_bursting_delta'] = merged['effect_prop'] - merged['baseline_prop']
    merged = merged.dropna(subset=['prop_bursting_delta'])
    merged['unit_id'] = merged['session_id']  # session-only bootstrap trick, matches heatmap
...
def forest_panel_with_violins(ax, stats_df, boots_dict, group_col, regions,
    ...
    ax.set_yticklabels([label_map.get(r, r) for r in regions], fontsize=14)
...
def apply_xlim_and_annotations(ax, xlim, sig_annots, drug_color):
    ...
    ax.set_xlim(xmin - pad, xmax + pad * 20)  # extra room for larger annotation text
    ...
    for (i, star_corr, star_unc, p_corr, d_val) in sig_annots:
        ...
        if star_corr:
            marker = star_corr
            fw, fs, alpha = 'bold', 16, 1.0
        elif star_unc:
            marker = f'({star_unc})'    # parens = uncorrected only
            fw, fs, alpha = 'normal', 13, 0.85
        else:
            marker = ''
            fw, fs, alpha = 'normal', 12, 0.7
        bits = []
        if marker:
            bits.append(marker)
...
def make_figure(deltas, group_col, regions, comparisons, delta_col,
    ...
    for row_idx, (drug_cond, ctrl_cond) in enumerate(comparisons):
        ...
        ax.set_xlabel(xlabel_str, fontsize=17)
...
def main():
    parser = argparse.ArgumentParser(
        description='Striatal subregion distribution plots (half-violin + point/CI) — configurable window',
    ...
    parser.add_argument('--out_dir',  default='distribution_plots_striatum_IR')
    parser.add_argument('--excel_out', default=None,
                        help='Excel output path (default: <out_dir>/striatum_stats_<window>.xlsx)')
    ...
    run_plot_group(
        deltas, prop_d,
        group_col='str_nucleus',
        row_order=STR_ROW_ORDER,
        label_map={k: k for k in STR_ROW_ORDER},
        title_main='Striatum',
        out_subdir=os.path.join(args.out_dir, f'striatum_{win_tag}'),
        nboots=args.nboots,
        min_units=args.min_units, min_sessions=args.min_sessions)

    # ── Excel: both uncorrected + FDR p-values, with the *** / (***) markers ──
    xlsx_path = args.excel_out or os.path.join(args.out_dir, f'striatum_stats_{win_tag}.xlsx')
