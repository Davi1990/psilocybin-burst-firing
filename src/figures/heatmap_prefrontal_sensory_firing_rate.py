...
Delta-of-delta coloring: (drug %Δ from baseline) − (control %Δ from baseline)

Usage
-----
  python heatmap_prefrontal_sensory.py --base_dir output_prestim_firing_rate
"""
...
# Share ONE colour bar across all firing-rate sub-panels of the figure
# (Prefrontal / Somatosensory / Visual and both comparisons). Only colour
...
def main():
    ...
    parser.add_argument('--output_dir',    default='heatmap_output_prefrontal_sensory')
    ...
    # Defer plotting so a shared colour scale can be computed across every
    # sub-panel of the figure before anything is drawn.
    plot_jobs = []
    vmax_by_metric = {}
    ...
    # ── Draw everything with a shared colour scale ───────────────────────────
    for _mt, _vm in vmax_by_metric.items():
        print(f"  Shared colour scale [{_mt}]: vmax = {_vm:.4g}  "
              f"(pass --vmax {_vm:.4g} to match this scale in the other scripts)")
    for _j in plot_jobs:
